...
//		Log.logger.info("temp:"+Time_Result[0]);
	...
	public static TimeUnit[] filterTimeUnit(final TimeUnit[] timeUnit) {
		if (timeUnit == null || timeUnit.length < 1) {
			return timeUnit;
		}
		final List<TimeUnit> list = new ArrayList<TimeUnit>();
		for (final TimeUnit t : timeUnit) {
	...
	private Pattern readModel(final String file) throws Exception {
		final ObjectInputStream in = new ObjectInputStream(
				new BufferedInputStream(new GZIPInputStream(new FileInputStream(file))));
		return readModel(in);
	}
	private Pattern readModel(final ObjectInputStream in) throws Exception {

		Pattern p = (Pattern) in.readObject();
		// Log.logger.info(p.pattern());
		return p = Pattern.compile(p.pattern());
	}
	
	public static void writeModel(final Object p, final String path) throws Exception {
	...
	public static void main(final String args[]) throws Exception {
		final String path = TimeNormalizer.class.getResource("").getPath();
		final String classPath = path.substring(0, path.length() - 1);
		Log.logger.info(classPath + "/TimeExp3.m");
		final TimeNormalizer normalizer = new TimeNormalizer(classPath + "/TimeExp3.m");
		
		normalizer.writeModel("", classPath);
		
//		normalizer.parse("今天天气");// 抽取时间：今天，十月一号到五号和十一月一号
//		TimeUnit[] unit = normalizer.getTimeUnit();
//		// Log.logger.info("unit[0].getTime():" + unit[0].getTime());
//		for (int i = 0; i < unit.length; i++) {
//			Log.logger.info(DateUtil.formatDateDefault(unit[i].getTime()) + "-" + unit[i].getIsAllDayTime());
//		}
//
//		normalizer.parse("十八号十九号今明天今天明天的天气明后天的天气未来几天");// 抽取时间
